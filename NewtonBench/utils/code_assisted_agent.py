...
def _format_budget_note(max_experiment_turns: int, max_points_per_experiment_turn: int, max_total_query_points: int) -> str:
    return (
        "\n\n**Experiment Budget Constraints:**\n"
        f"- You may use at most {max_experiment_turns} experiment turns in total.\n"
        f"- In one experiment turn, you may request at most {max_points_per_experiment_turn} query points.\n"
        f"- Across the whole task, you may request at most {max_total_query_points} total query points.\n"
        "- If you request more than the remaining budget, only the earliest allowed points will be executed."
    )
...
def conduct_code_assisted_exploration(
    ...
    max_turns: int = 10,
    max_experiment_turns: int = 5,
    max_points_per_experiment_turn: int = 4,
    max_total_query_points: int = 20,
) -> Dict[str, Any]:
    """
    ...
    # max_turns is passed as a parameter (default 10)
    # Create code assisted agent-specific system prompt
    system_prompt = create_code_assisted_system_prompt(
        module,
        difficulty,
        system,
        max_turns,
        max_experiment_turns=max_experiment_turns,
        max_points_per_experiment_turn=max_points_per_experiment_turn,
        max_total_query_points=max_total_query_points,
    )
            ...
            try:
                ...
                if code_result['has_python_tag'] and python_pos > experiment_pos:
                    ...
                    if response and "<run_experiment>" in response and "</run_experiment>" in response:
                        remaining_query_points = max_total_query_points - num_experiments
                        remaining_experiment_turns = max_experiment_turns - num_experiment_turns
                        if remaining_query_points <= 0 or remaining_experiment_turns <= 0:
                            budget_message = (
                                "Experiment budget exhausted. You cannot run more experiments. "
                                "Use prior results, optional <python> analysis, and submit <final_law> when ready."
                            )
                            if messages and messages[-1]["role"] == "user":
                                messages[-1]["content"] += "\n\n" + budget_message
                                chat_history[-1]["content"] += "\n\n" + budget_message
                                ...
                                chat_history.append({"role": "user", "content": budget_message})
                        else:
                            print(f"[Code Assisted Trial {trial_id}] Running experiment {num_experiments} in turn {turn + 1}")
                            allowed_points = min(max_points_per_experiment_turn, remaining_query_points)
                            experiment_result, executed_points, requested_points = run_experiment_from_response(
                                module,
                                response,
                                system,
                                noise_level,
                                difficulty,
                                law_version,
                                max_experiment_points=allowed_points,
                            )
                            if executed_points > 0:
                                num_experiments += executed_points
                                num_experiment_turns += 1
                            if experiment_result:
                            # Add experiment results to conversation
                                experiment_message = format_experiment_results(experiment_result)
                                if requested_points > executed_points:
                                    experiment_message += (
                                        f"\n\n[Budget note] You requested {requested_points} points, "
                                        f"but only {executed_points} were executed because of the experiment budget."
                                    )
                                if messages and messages[-1]["role"] == "user":
                                    messages[-1]["content"] += "\n\n" + experiment_message
    ...
    return {
        ...
        "num_experiment_turns": num_experiment_turns,
        "exploration_mode": "code_assisted_agent"
    }

def create_code_assisted_system_prompt(
    module,
    difficulty: str,
    system: str,
    max_turns: int,
    *,
    max_experiment_turns: int,
    max_points_per_experiment_turn: int,
    max_total_query_points: int,
) -> str:
    """
...
1. **Math calculation**:
    ...
    - **Always use print()** for any value or comparison you need to see (e.g. predicted vs measured, differences, exponents). Code that only computes a value without printing it produces no visible output and you will not see the result.
2. **Enhanced Tool Use**:
...
- Always respond with the appropriate tag when submitting experiments or final laws. The environment will handle execution and feedback."""
    ...
    return code_assisted_system_prompt + _format_budget_note(
        max_experiment_turns=max_experiment_turns,
        max_points_per_experiment_turn=max_points_per_experiment_turn,
        max_total_query_points=max_total_query_points,
    )

def run_experiment_from_response(
    module,
    response: str,
    system: str,
    noise_level: float,
    difficulty: str,
    law_version: str = None,
    max_experiment_points: Optional[int] = None,
    ...
    try:
        ...
        try:
            ...
            if isinstance(exp_data, list):
                ...
                requested_points = len(exp_data)
                if max_experiment_points is not None:
                    exp_data = exp_data[:max_experiment_points]
                results = []
                for exp in exp_data:
                    if hasattr(module, 'run_experiment_for_module'):
                        ...
                        results.append(None)
                return results, len(exp_data), requested_points
            else:
                # Single experiment
                requested_points = 1
                if max_experiment_points is not None and max_experiment_points <= 0:
                    return None, 0, requested_points
                if hasattr(module, 'run_experiment_for_module'):
                    result = module.run_experiment_for_module(
                        **exp_data, 
                        noise_level=noise_level, 
                    ...
                    return result, 1, requested_points
                else:
                    return None, 0, requested_points
