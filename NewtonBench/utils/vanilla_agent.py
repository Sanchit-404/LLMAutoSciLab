...
def _format_budget_note(max_experiment_turns: int, max_points_per_experiment_turn: int, max_total_query_points: int) -> str:
    return (
        ...
        "- Do not submit `<final_law>` until you have exhausted the remaining experiment budget or the system explicitly tells you the budget is exhausted.\n"
        "- While budget remains, prefer using the full allowed number of query points in each experiment turn."
    )
...
def conduct_exploration(
    ...
    """
    base_prompt = BASE_PROMPT.format(max_turns=max_turns) + _format_budget_note(
    ...
    for turn in range(max_turns):
        ...
        if is_submitted:
            remaining_query_points = max_total_query_points - num_experiments_run
            remaining_experiment_turns = max_experiment_turns - num_experiment_turns_used
            if remaining_query_points > 0 and remaining_experiment_turns > 0:
                forced_continue = (
                    "You still have unused experiment budget remaining. "
                    f"There are {remaining_query_points} query points and {remaining_experiment_turns} experiment turns left. "
                    "Do not submit <final_law> yet. In your next response, use <run_experiment> and request as many informative points as allowed "
                    f"(up to {min(max_points_per_experiment_turn, remaining_query_points)} points this turn)."
                )
                messages.append({"role": "user", "content": forced_continue})
                continue
            return {
                "status": "completed",
                ...
                "num_experiment_turns": num_experiment_turns_used,
        ...
        if experiments_to_run:
                ...
                messages.append(
                    {
                        ...
                        "content": (
                            ...
                            "Use prior results to reason and submit <final_law> when ready."
                        ),
                    }
                )
                continue
            allowed = min(len(experiments_to_run), max_points_per_experiment_turn, remaining_query_points)
            experiments_to_execute = experiments_to_run[:allowed]
            num_experiment_turns_used += 1
            num_experiments_run += len(experiments_to_execute)
            results = []
            for exp in experiments_to_execute:
            ...
            if allowed < len(experiments_to_run):
                output_str += (
                    f"\n\n[Budget note] You requested {len(experiments_to_run)} points, "
                    f"but only {allowed} were executed because of the experiment budget."
                )
