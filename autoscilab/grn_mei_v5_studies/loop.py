...
from autoscilab.grn_mei_v5_studies.graph_search import (
    GraphHypothesisFit,
    GraphHypothesisLineage,
    build_hypothesis_lineages,
    select_committee_from_lineages,
)
from autoscilab.grn_mei_v5_studies.hypothesis_dist import GraphHypothesisDistribution
from autoscilab.grn_mei_v5_studies.schemas import (
    GraphTranslationProposal,
    HypothesisReasoningProposal,
    SampledGraphHypothesis,
...
SINGLE_HYPOTHESIS_SYSTEM_PROMPT = """You are a scientific reasoning agent for mechanistic gene regulatory network discovery.

Goal:
- infer one plausible signed regulatory mechanism from perturbation-response experiments

Rules:
1. Return exactly one concise natural-language mechanism hypothesis.
2. The hypothesis must describe causal structure over nodes signal, A, B, C, R.
3. Do not output graph edges yet.
4. Prefer sparse, falsifiable mechanisms with an explicit route from signal to C.
5. Use the observed data as the primary evidence and the memory summary as historical context.
"""
...
class GRNMEIGraphConfig:
    ...
    hypothesis_conditioned_acquisition: bool = True
    iterative_refinement_enabled: bool = True
    confidence_feedback_enabled: bool = True
    ...
    observation_mode: str = "full_panel"
...
def _store_to_table(store: ExperimentStore, max_rows: int = 20) -> str:
    return _store_to_table_with_mode(store, "full_panel", max_rows=max_rows)


def _store_to_table_with_mode(store: ExperimentStore, observation_mode: str, max_rows: int = 20) -> str:
    rows = store.get_all()[-max_rows:]
    if not rows:
        return "No experiments yet."
    if observation_mode == "reporter_only":
        header = "signal | pert_A | pert_B | pert_C | pert_R | reporter"
    elif observation_mode == "reporter_plus_ar":
        header = "signal | pert_A | pert_B | pert_C | pert_R | reporter | A | R"
    else:
    ...
    for row in rows:
        ...
        if observation_mode == "reporter_plus_ar":
            vals.extend([
        ...
        elif observation_mode == "full_panel":
            vals.extend([
                f"{float(panel.get('A', row.metadata.get('states', {}).get('A', float('nan')))):.6g}",
...
def _build_reasoning_prompt(
    ...
    return (
        ...
        f"RECENT DATA:\n{_store_to_table_with_mode(store, cfg.observation_mode, max_rows=25)}\n\n"
        f"HYPOTHESIS FIT SUMMARY:\n{lineage_summary}\n\n"
        f"{f'MEMORY SUMMARY:\\n{memory_str}\\n\\n' if memory_str else ''}"
        f"{f'CONFIDENCE SUMMARY:\\n{confidence_str}\\n\\n' if confidence_str else ''}"
        f"{f'ENSEMBLE GRAPH DISTRIBUTION:\\n{ensemble_summary}\\n\\n' if ensemble_summary else ''}"
        f"{extra}"
...
def _build_sampling_prompt(
    ...
    return (
        ...
        f"{f'CONFIDENCE SUMMARY:\\n{confidence_str}\\n\\n' if confidence_str else ''}"
        f"BUDGET REMAINING: {budget_remaining}\n\n"
        "Return exactly one plausible natural-language mechanism hypothesis, with a short rationale and confidence.\n"
...
class GRNMEIGraphLoop:
    def __init__(
        ...
        if config.ensemble_mode:
            self._ensemble = EnsembleClient(
                base_url=config.ensemble_url or base_url or "http://localhost:8001/v1",
                model=config.ensemble_model or config.llm_model,
                k=config.ensemble_k,
        ...
        self._last_distribution: GraphHypothesisDistribution | None = None

    def _observed_marker_nodes(self) -> list[str]:
        if self._cfg.observation_mode == "reporter_only":
            return []
        if self._cfg.observation_mode == "reporter_plus_ar":
            return ["A", "R"]
        return ["A", "B", "C", "R"]

    def _confidence_summary(self) -> str:
        if not self._cfg.confidence_feedback_enabled or self._last_confidence <= 0.0:
            return ""
        if self._last_confidence >= 0.8:
            label = "high"
        elif self._last_confidence >= 0.5:
            label = "moderate"
        else:
            label = "low"
        return (
            f"Most recent mechanistic confidence: {self._last_confidence:.2f} ({label}).\n"
            "If confidence is already high, prioritize refinement or validation over broad exploration."
        )
    ...
    def _reason_hypotheses(
        ...
        prompt = _build_reasoning_prompt(
            ...
            confidence_str=self._confidence_summary(),
    ...
    def _sample_single_hypotheses(
        self,
        lineages: list[GraphHypothesisLineage],
        n_samples: int,
    ) -> list[SampledGraphHypothesis]:
        prompt = _build_sampling_prompt(
            self._cfg,
            self._oracle,
            self._store,
            self._current_hypothesis,
        ...
        if self._ensemble is not None:
            self._llm_calls += n_samples
            try:
                return self._ensemble.complete_json_k(
                    messages=[{"role": "user", "content": prompt}],
                    system=SINGLE_HYPOTHESIS_SYSTEM_PROMPT,
                    schema=SampledGraphHypothesis,
                    k_override=n_samples,
                    min_valid=1,
                )
            except Exception as exc:
                print(
                    "[GRN-MEI-Graph] ensemble sampling fallback -> sequential single-call sampling "
                    f"({type(exc).__name__}: {exc})"
                )

        samples: list[SampledGraphHypothesis] = []
        for _ in range(n_samples):
            ...
            samples.append(
                self._client.complete_json(
                    ...
                    system=SINGLE_HYPOTHESIS_SYSTEM_PROMPT,
                    schema=SampledGraphHypothesis,
                    tool_name="sample_grn_hypothesis",
                )
            )
        return samples

    def _translate_sampled_hypothesis(
        self,
        sample: SampledGraphHypothesis,
    ) -> dict[str, Any]:
        payload = {
            "hypothesis_id": "h1",
            "text": sample.text,
        }
        prompt = _build_single_translation_prompt(payload)
        last_error = None
        for _attempt in range(1, self._cfg.max_llm_retry_attempts + 1):
            self._llm_calls += 1
            try:
                translated = self._client.complete_json(
                    messages=[{"role": "user", "content": prompt}],
                ...
                return {
                    "hypothesis_text": sample.text,
                    "rationale": sample.rationale,
                    "edges": [edge.model_dump() for edge in translated.translation.edges],
                }
            except Exception as exc:
                last_error = str(exc)
                prompt += (
                    ...
                    "Return exactly one valid translation for this same mechanism."
                )
        raise RuntimeError(last_error or "Failed to translate sampled GRN hypothesis.")

    def _sample_graph_distribution(
        self,
        lineages: list[GraphHypothesisLineage],
    ) -> GraphHypothesisDistribution:
        k_batch = max(1, self._cfg.ensemble_k)
        adaptive = self._cfg.ensemble_adaptive
        k_limit = self._cfg.ensemble_k_max if adaptive else k_batch
        prev_entropy: float | None = None
        total = 0
        collected: list[dict[str, Any]] = []

        while total < k_limit:
            batch = min(k_batch, k_limit - total)
            samples = self._sample_single_hypotheses(lineages, batch)
            for sample in samples:
                try:
                    translated = self._translate_sampled_hypothesis(sample)
                    translated["source_idx"] = len(collected)
            ...
            if not collected:
                continue
            dist = GraphHypothesisDistribution.from_samples(collected)
            if not adaptive:
            ...
            if prev_entropy is not None:
                ...
                print(
                    f"[GRN-MEI-Graph] adaptive graph sampling k={total}/{k_limit} "
                    f"entropy={entropy:.3f} delta={delta:.3f}"
                )
                if delta < self._cfg.ensemble_stability_threshold:
                    self._last_distribution = dist
                    return dist
            prev_entropy = entropy

        if not collected:
            raise RuntimeError("Failed to obtain any valid sampled GRN graph hypotheses.")
        dist = GraphHypothesisDistribution.from_samples(collected)
        self._last_distribution = dist
        return dist
    ...
    def _build_lineages(self, reasoning: HypothesisReasoningProposal, translation: GraphTranslationProposal) -> list[GraphHypothesisLineage]:
        ...
        structured = []
        max_edit_distance = self._cfg.max_graph_edit_distance if self._cfg.iterative_refinement_enabled else 0
        ...
        for item in structured:
            try:
                build_hypothesis_lineages(
                    ...
                    use_internal_state=self._cfg.use_internal_state and self._cfg.observation_mode != "reporter_only",
                    observed_marker_nodes=self._observed_marker_nodes(),
                )
    ...
    def _update_memory(self, iteration: int, lineages: list[GraphHypothesisLineage]) -> None:
        if not self._cfg.memory_enabled or not lineages:
            return
        n_data = len(self._store)
        best_rmsle = float(lineages[0].best_fit.train_rmsle)
        for lineage in lineages[: min(5, len(lineages))]:
            rmsle = float(lineage.best_fit.train_rmsle)
            self._memory.add_hypothesis_score(
                iteration=iteration,
                hypothesis=lineage.hypothesis_text,
                fit_rmsle=rmsle,
                n_data=n_data,
                status="uncertain",
            )
            if rmsle <= best_rmsle * 1.15:
            ...
            elif rmsle >= best_rmsle * 1.75:
                self._memory.add_negative_evidence(
                    iteration=iteration,
                    hypothesis=lineage.hypothesis_text,
                    evidence=(
                        f"Graph lineage fit RMSLE={rmsle:.4f} was substantially worse than the current best "
                        f"lineage RMSLE={best_rmsle:.4f}."
                    ),
                    gt_rmsle=rmsle,
                )
    ...
    def _reason_translate_until_valid(
        ...
        GraphHypothesisDistribution | None,
    ]:
        correction: str | None = None
        for attempt in range(1, self._cfg.max_llm_retry_attempts + 1):
            distribution: GraphHypothesisDistribution | None = None
            use_ensemble = self._cfg.ensemble_mode
            if (
                use_ensemble
                and self._cfg.confidence_feedback_enabled
                and self._cfg.ensemble_confidence_gate < 1.0
                and self._last_confidence >= self._cfg.ensemble_confidence_gate
            ):
                use_ensemble = False
                print(
                    f"[GRN-MEI-Graph] skip adaptive sampling: confidence={self._last_confidence:.2f} "
                    f">= gate {self._cfg.ensemble_confidence_gate:.2f}"
                )

            if use_ensemble:
                distribution = self._sample_graph_distribution(last_lineages)
                reasoning = self._reason_hypotheses(
                    last_lineages,
                    ensemble_summary=distribution.synthesis_prompt,
                    extra_instruction=correction,
                )
                self._last_confidence = max(reasoning.confidence, distribution.agreement_score)
            else:
                reasoning = self._reason_hypotheses(last_lineages, extra_instruction=correction)
                self._last_confidence = reasoning.confidence
            ...
            if len(self._store) < self._cfg.min_data_for_graph_fit:
                return reasoning, translation, [], [], distribution
            ...
            if len(lineages) >= 2 and len(committee) >= 2:
                return reasoning, translation, lineages, committee, distribution
    ...
    def _run_region(self, region_bounds: dict[str, list[float]], n_points: int, fits: list[GraphHypothesisFit]) -> list[dict[str, float]]:
        ...
        if len(fits) < 2 or not self._cfg.allow_disagreement:
    ...
    def _select_points_with_llm_retries(
        ...
        if not self._cfg.hypothesis_conditioned_acquisition:
            full_region = {
                key: [float(bounds[0]), float(bounds[1])]
                for key, bounds in self._oracle.parameter_bounds.items()
            }
            return reasoning, self._run_region(full_region, batch_cap, committee)[:batch_cap]
        ...
        for attempt in range(1, self._cfg.max_llm_retry_attempts + 1):
            ...
            current_reasoning = self._reason_hypotheses(
                lineages,
                ensemble_summary=self._last_distribution.synthesis_prompt if self._last_distribution is not None else "",
                extra_instruction=correction,
            )
    ...
    def run(self) -> GRNMEIGraphResult:
        ...
        while len(self._store) < self._cfg.budget:
            ...
            reasoning, translation, lineages, committee, distribution = self._reason_translate_until_valid(last_lineages)
            if lineages:
                last_lineages = lineages
                last_committee = committee
                self._update_memory(iteration, lineages)
                print(
            ...
            hist_row = {
                ...
                "effective_confidence": self._last_confidence,
                "search_regions": [region.model_dump() for region in reasoning.search_regions],
                "selected_points": selected_points[:batch_cap],
                "graph_distribution": {
                    "n_samples": len(distribution.hypotheses),
                    "n_unique_structures": distribution.n_unique_structures(),
                    "agreement_score": distribution.agreement_score,
                    "entropy": distribution.entropy(),
                    "summary": distribution.synthesis_prompt,
        ...
        if not last_lineages:
            _reasoning, _translation, last_lineages, last_committee, _distribution = self._reason_translate_until_valid([])
        ...
        summary = {
            ...
            "effective_confidence": self._last_confidence,
            "graph_distribution": {
                "n_samples": len(self._last_distribution.hypotheses),
                "n_unique_structures": self._last_distribution.n_unique_structures(),
                "agreement_score": self._last_distribution.agreement_score,
                "entropy": self._last_distribution.entropy(),
                "summary": self._last_distribution.synthesis_prompt,
