...
def _extract_targets(
    ...
    observed_marker_nodes: list[str] | None = None,
    ...
    observed_nodes = [node for node in (observed_marker_nodes or _TARGET_NODES) if node in _TARGET_NODES]
    if not observed_nodes:
        return X, np.array(y_target, dtype=float), None
    state_targets = {node: [] for node in observed_nodes}
    has_states = True
    for result in store.get_all():
        panel = result.metadata.get("marker_panel")
...
def _loss(
    ...
    state_losses = [
        _rmsle(np.maximum(states_hat[node], 1e-6), np.maximum(target, 1e-6))
        for node, target in state_targets.items()
    ]
...
def fit_graph_hypothesis(
    ...
    X, y, state_targets = _extract_targets(
        ...
        observed_marker_nodes=observed_marker_nodes,
    )
    if len(X) == 0:
