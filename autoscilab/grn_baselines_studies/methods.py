...
from autoscilab.grn_mei_v5_studies.loop import GRNMEIGraphConfig, GRNMEIGraphLoop
...
def _deterministic_marker_noise(
    domain: str,
    difficulty: str,
    law_version: str,
    ...
    if noise_level == 0.0:
        return 0.0
    seed_str = (
        f"{domain}|{difficulty}|{law_version}|marker={marker}|"
        + "|".join(f"{k}={v:.8g}" for k, v in sorted(params.items()))
    )
    ...
    for result in store.get_all():
        ...
        if not isinstance(panel, dict):
            ...
            if not isinstance(states, dict):
                raise ValueError("Offline graph baselines require oracle marker/state metadata.")
            panel = {
                "A": float(states["A"]),
                "B": float(states["B"]),
                "C": float(states["C"]),
        ...
        law_version = str(result.metadata.get("law_version", "v0"))
        noise_level = float(result.noise_level)
        noisy_panel = {}
        for marker in STATE_NODES:
            base_value = float(panel[marker])
            eps = _deterministic_marker_noise(
            ...
            noisy_panel[marker] = max(base_value * (1.0 + eps), 1e-6)
        rows.append([
...
def run_genie3(
    ...
    return BaselineResult(
        ...
        extras={"design": dataset_source, "observation_mode": "noisy_marker_panel", "implementation": "internal_genie3_like"},
    )
...
def run_gies(
    ...
    return BaselineResult(
        ...
        extras={
            ...
            "observation_mode": "noisy_marker_panel",
